...
function FindSoundLength(musicFile)
    local dired = "/Themes/pitchBlende/Sounds/"
    local auPath = dired .. musicFile
    local auSecI, auSecJ = string.find(musicFile,"sec;")
    local auLoopI, auLoopJ = string.find(musicFile,",loop")
    local auEndI, auEndJ = string.find(musicFile,"].ogg")
    local auHasLoop = (string.find(musicFile,",loop") and true or false)
    local auLen = tonumber(string.sub(musicFile, (auSecJ+1), (auHasLoop and (auLoopI-1) or (auEndI-1))))
    return auLen, auHasLoop
end
...
function save_conf_file(tbl, fname)
    local tstr = lua_table_to_string(tbl, "\t", nil)
    tstr = "return " .. tstr
    write_str_to_file(tstr, fname, fname)
end

function ShowCredits()
    MESSAGEMAN:Broadcast("Credits")
end

function CycleSongIni()
    songIni = load_conf_file(THEME:GetPathO("Menu", "selectedsong"))
    songIniCurChoice = songIni.SongSelect.CurrentChoice
    songIni1Choice = songIni.SongSelect.Choice1
    songIni2Choice = songIni.SongSelect.Choice2
    if tostring(songIniCurChoice) == tostring(songIni1Choice)
    then
        songIni.SongSelect.CurrentChoice = songIni2Choice
        save_conf_file(songIni, THEME:GetPathO("Menu", "selectedsong"))
        local songChoice = songIni2Choice
        local songLen, songLoop = FindSoundLength(songChoice)
        MESSAGEMAN:Broadcast("SongModified", { choice = songChoice, length = songLen, hasLoop = songLoop })
        return true
    elseif tostring(songIniCurChoice) == tostring(songIni2Choice)
    then
        songIni.SongSelect.CurrentChoice = songIni1Choice
        save_conf_file(songIni, THEME:GetPathO("Menu", "selectedsong"))
        local songChoice = songIni1Choice
        local songLen, songLoop = FindSoundLength(songChoice)
        MESSAGEMAN:Broadcast("SongModified", { choice = songChoice, length = songLen, hasLoop = songLoop })
        return true
    end
    Warn("Couldn't change BGM!")
end
