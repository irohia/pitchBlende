...
local ratios = {
    LeftGap = (SCREEN_CENTER_X / 2) / SCREEN_WIDTH,
                ...
                UIElements.QuadButton(1) .. {
                    ...
                    MouseDownCommand = function(self, params)
                        ...
                        elseif params.event == "DeviceButton_right mouse button" then
                            local index = self:GetParent().index
                            local distance = math.floor(index - numWheelItems / 2) - 1
                            if distance == 0 or distance ~= 0 then
...
t[#t+1] = Def.ActorFrame {
    ...
    Def.ActorFrame {
        ...
        LoadFont("Common Normal") .. {
            ...
            InitCommand = function(self)
                self:halign(0):valign(0)
                self:xy(actuals.HeaderMTextLeftGap, (SCREEN_HEIGHT-175) + (actuals.HeaderMText1UpperGap))
        ...
        LoadFont("Common Normal") .. {
            ...
            InitCommand = function(self)
                self:halign(0):valign(0)
                self:xy(actuals.HeaderMTextLeftGap, (SCREEN_HEIGHT-175) + (actuals.HeaderMText2UpperGap))
                self:zoom(wheelHeaderMTextSize)
                self:maxwidth((actuals.BannerWidth - actuals.HeaderMTextLeftGap) / wheelHeaderMTextSize)
                self:diffusealpha(1)
                self:draworder(9999)
        ...
        LoadFont("Common Normal") .. {
            ...
            InitCommand = function(self)
                self:halign(0):valign(0)
                self:xy(actuals.HeaderMTextLeftGap, (SCREEN_HEIGHT-175) + (actuals.HeaderMText3UpperGap))
                self:zoom(wheelHeaderMTextSize)
                self:maxwidth((actuals.BannerWidth - actuals.HeaderMTextLeftGap) / wheelHeaderMTextSize)
                self:diffusealpha(1)
                self:draworder(9999)
        ...
        Def.ActorFrame {
            ...
            Def.Quad {
                ...
                InitCommand = function(self)
                    ...
                    self:y((SCREEN_HEIGHT-175) + (actuals.HeaderHeight / 8))
            ...
            Def.Quad {
                ...
                InitCommand = function(self)
                    self:halign(0):valign(0)
                    self:x(actuals.HeaderMTextLeftGap)
                    self:y((SCREEN_HEIGHT-175) + (actuals.HeaderHeight - actuals.HeaderHeight / 8))
            ...
            LoadFont("Common Normal") .. {
                ...
                InitCommand = function(self)
                    ...
                    self:y((SCREEN_HEIGHT-175) + (actuals.HeaderHeight / 8 - graphBoundOffset))
                    self:rotationz(-45)
            ...
            LoadFont("Common Normal") .. {
                ...
                InitCommand = function(self)
                    self:halign(1)
                    self:x(actuals.HeaderMTextLeftGap - graphBoundOffset / 2)
                    self:y((SCREEN_HEIGHT-175) + (actuals.HeaderHeight / 8 * 7 - graphBoundOffset))
                    self:rotationz(-45)
                    self:zoom(graphBoundTextSize)
            ...
            Def.ActorMultiVertex {
                ...
                InitCommand = function(self)
                    self:x(actuals.HeaderMTextLeftGap)
                    self:y((SCREEN_HEIGHT-175) + (actuals.HeaderHeight / 8 * 7))
                    self:draworder(9999)
            ...
            Def.Quad {
                Name = "Box",
                InitCommand = function(self)
                    self:zoomto(SCREEN_WIDTH+256, 184)
                    self:xy(0, SCREEN_HEIGHT-128)
                    self:diffuse(color("#404040"))
                    self:diffusealpha(1)
                    self:visible(true)
                    self:draworder(9998)
                end
